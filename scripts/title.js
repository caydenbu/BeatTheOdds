...
async function GlitchAnimation() {
    for (let i = 0; i < targetWord.length; i++) {
        ...
        for (let j = 0; j < alphabet.length; j++) {
            ...
            currentText = currentText.substring(0, i) + currentLetter + currentText.substring(i + 1);
            startButton.innerHTML = currentText;
        }
        await wait(50); // Adjust the delay as needed
        let currentText = startButton.innerHTML;
        currentText = currentText.substring(0, i) + targetWord[i] + currentText.substring(i + 1);
        startButton.innerHTML = currentText;
    }
}